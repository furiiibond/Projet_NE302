...
int matchedCommand(const char **ptr, const char *end, node **node_ptr) {
	...
	// $debut est un pointeur vers la première lettre du tag (le début du mot)
	const char * debut = (*ptr);
	// pour avoir la fin du mot, il suffit d'avancer le pointeur *ptr jusqu'au prochain espace
	// (dans la grammaire tous les éléments sont séparés par des espaces)
...
int loopAlgoCalls(int min, int max,const  char *debut,const  char *fin, node **noeud) {
	...
	char *backup = mem;
	char *backup2 = mem; // si on a validé moins de fois que min
	...
	//		Si on est sorti car on a atteint le nombre max d'ittération alors le pointeur mem
